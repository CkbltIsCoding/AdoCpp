...
namespace AdoCpp
{
    ...
    Vector2<T> Vector2<T>::rotatedBy(Angle phi) const
    {
        ...
        T cos = std::cos(static_cast<T>(phi.rad()));
        T sin = std::sin(static_cast<T>(phi.rad()));
    ...
    Vector2<T>::Vector2(T r, Angle phi) :
        x(r * static_cast<T>(std::cos(phi.rad()))), y(r * static_cast<T>(std::sin(phi.rad())))
    {
        static_assert(std::is_floating_point_v<T>,
...
template class AdoCpp::Vector2<float>;
template class AdoCpp::Vector2<double>;
template class AdoCpp::Vector2<long double>;
