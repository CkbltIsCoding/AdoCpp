...
namespace AdoCpp
{
    ...
    constexpr Color::Color(std::string color)
    {
        const auto charToDigit16 = [](char c)
            {
    ...
    constexpr std::string Color::toString(bool hash, bool uppercase, bool ignoreAlpha) const
    {
        std::string str;
        if (hash) str.push_back('#');
        const auto digit16ToChar = [uppercase](std::uint8_t v)
            {
